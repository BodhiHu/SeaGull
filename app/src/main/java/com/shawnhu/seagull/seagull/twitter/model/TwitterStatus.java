...
import com.shawnhu.seagull.utils.JSON.ParcelableWithJSON;
...
public class TwitterStatus implements ParcelableWithJSON, Comparable<TwitterStatus> {

    public static final Parcelable.Creator<TwitterStatus> CREATOR = new Parcelable.Creator<TwitterStatus>() {
        @Override
        public TwitterStatus createFromParcel(final Parcel in) {
            return new TwitterStatus(in);
        }

        @Override
        public TwitterStatus[] newArray(final int size) {
            return new TwitterStatus[size];
        }
    };

    public static final JSONParcelable.Creator<TwitterStatus> JSON_CREATOR = new JSONParcelable.Creator<TwitterStatus>() {
        @Override
        public TwitterStatus createFromParcel(final JSONParcel in) {
            return new TwitterStatus(in);
        }
    ...
    public static final Comparator<TwitterStatus> TIMESTAMP_COMPARATOR = new Comparator<TwitterStatus>() {

        @Override
        public int compare(final TwitterStatus object1, final TwitterStatus object2) {
    ...
    public static final Comparator<TwitterStatus> REVERSE_ID_COMPARATOR = new Comparator<TwitterStatus>() {
    ...
    public final TwitterUserMention[] mentions;

    public final TwitterMedia[] medias;

    public TwitterStatus(final ContentValues values) {
        id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.STATUS_ID, -1);
        account_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.ACCOUNT_ID, -1);
        timestamp = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.STATUS_TIMESTAMP, -1);
        user_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.USER_ID, -1);
        retweet_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.RETWEET_ID, -1);
        retweet_timestamp = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.RETWEET_TIMESTAMP, -1);
        retweeted_by_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.RETWEETED_BY_USER_ID, -1);
        user_name = values.getAsString(TweetStore.Statuses.USER_NAME);
        user_screen_name = values.getAsString(TweetStore.Statuses.USER_SCREEN_NAME);
        text_html = values.getAsString(TweetStore.Statuses.TEXT_HTML);
        text_plain = values.getAsString(TweetStore.Statuses.TEXT_PLAIN);
        user_profile_image_url = values.getAsString(TweetStore.Statuses.USER_PROFILE_IMAGE_URL);
        is_favorite = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_FAVORITE, false);
        is_retweet = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_RETWEET, false);
        is_gap = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_GAP, false);
        location = TwitterLocation.fromString(values.getAsString(TweetStore.Statuses.LOCATION));
        user_is_protected = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_PROTECTED, false);
        user_is_verified = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_VERIFIED, false);
        in_reply_to_status_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.IN_REPLY_TO_STATUS_ID, -1);
        in_reply_to_user_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.IN_REPLY_TO_USER_ID, -1);
        in_reply_to_name = values.getAsString(TweetStore.Statuses.IN_REPLY_TO_USER_NAME);
        in_reply_to_screen_name = values.getAsString(TweetStore.Statuses.IN_REPLY_TO_USER_SCREEN_NAME);
        my_retweet_id = ContentValuesUtils.getAsLong(values, TweetStore.Statuses.MY_RETWEET_ID, -1);
        retweeted_by_name = values.getAsString(TweetStore.Statuses.RETWEETED_BY_USER_NAME);
        retweeted_by_screen_name = values.getAsString(TweetStore.Statuses.RETWEETED_BY_USER_SCREEN_NAME);
        source = values.getAsString(TweetStore.Statuses.SOURCE);
        retweet_count = ContentValuesUtils.getAsInteger(values, TweetStore.Statuses.RETWEET_COUNT, 0);
        favorite_count = ContentValuesUtils.getAsInteger(values, TweetStore.Statuses.FAVORITE_COUNT, 0);
        text_unescaped = values.getAsString(TweetStore.Statuses.TEXT_UNESCAPED);
        medias = TwitterMedia.fromJSONString(values.getAsString(TweetStore.Statuses.MEDIAS));
        is_possibly_sensitive = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_POSSIBLY_SENSITIVE, false);
        user_is_following = ContentValuesUtils.getAsBoolean(values, TweetStore.Statuses.IS_FOLLOWING, false);
        mentions = TwitterUserMention.fromJSONString(values.getAsString(TweetStore.Statuses.MENTIONS));
    ...
    public TwitterStatus(final Cursor c, final CursorIndices idx) {
        ...
        location = idx.location != -1 ? new TwitterLocation(c.getString(idx.location)) : null;
        ...
        mentions = idx.mentions != -1 ? TwitterUserMention.fromJSONString(c.getString(idx.mentions)) : null;
        first_media = idx.first_media != -1 ? c.getString(idx.first_media) : null;
    }

    public TwitterStatus(final JSONParcel in) {
        ...
        medias = in.readParcelableArray("medias", TwitterMedia.JSON_CREATOR);
        location = in.readParcelable("location", TwitterLocation.JSON_CREATOR);
        ...
        mentions = in.readParcelableArray("mentions", TwitterUserMention.JSON_CREATOR);
        first_media = medias != null && medias.length > 0 ? medias[0].url : null;
    }

    public TwitterStatus(final Parcel in) {
        id = in.readLong();
        ...
        mentions = in.createTypedArray(TwitterUserMention.CREATOR);
        first_media = medias != null && medias.length > 0 ? medias[0].url : null;
    }

    public TwitterStatus(final twitter4j.Status orig, final long account_id, final boolean is_gap) {
        this.is_gap = is_gap;
        this.account_id = account_id;
        id = orig.getId();
        timestamp = getTime(orig.getCreatedAt());
        is_retweet = orig.isRetweet();
        final twitter4j.Status retweeted_status = orig.getRetweetedStatus();
        ...
        final twitter4j.Status status = retweeted_status != null ? retweeted_status : orig;
        ...
        text_html = Utils.formatStatusText(status);
        medias = TwitterMedia.fromEntities(status);
        text_plain = status.getText();
        retweet_count = status.getRetweetCount();
        favorite_count = status.getFavoriteCount();
        in_reply_to_name = Utils.getInReplyToName(status);
        in_reply_to_screen_name = status.getInReplyToScreenName();
        in_reply_to_status_id = status.getInReplyToStatusId();
        in_reply_to_user_id = status.getInReplyToUserId();
        source = status.getSource();
        location = new TwitterLocation(status.getGeoLocation());
        is_favorite = status.isFavorited();
        text_unescaped = HtmlEscapeHelper.toPlainText(text_html);
        my_retweet_id = retweeted_by_id == account_id ? id : -1;
        is_possibly_sensitive = status.isPossiblySensitive();
        mentions = TwitterUserMention.fromUserMentionEntities(status.getUserMentionEntities());
        first_media = medias != null && medias.length > 0 ? medias[0].url : null;
    }

    @Override
    public int compareTo(final TwitterStatus another) {
    ...
    public boolean equals(final Object obj) {
        ...
        final TwitterStatus other = (TwitterStatus) obj;
    ...
    public static final class CursorIndices {
        ...
        public CursorIndices(final Cursor cursor) {
            _id = cursor.getColumnIndex(TweetStore.Statuses._ID);
            account_id = cursor.getColumnIndex(TweetStore.Statuses.ACCOUNT_ID);
            status_id = cursor.getColumnIndex(TweetStore.Statuses.STATUS_ID);
            status_timestamp = cursor.getColumnIndex(TweetStore.Statuses.STATUS_TIMESTAMP);
            user_name = cursor.getColumnIndex(TweetStore.Statuses.USER_NAME);
            user_screen_name = cursor.getColumnIndex(TweetStore.Statuses.USER_SCREEN_NAME);
            text_html = cursor.getColumnIndex(TweetStore.Statuses.TEXT_HTML);
            text_plain = cursor.getColumnIndex(TweetStore.Statuses.TEXT_PLAIN);
            text_unescaped = cursor.getColumnIndex(TweetStore.Statuses.TEXT_UNESCAPED);
            user_profile_image_url = cursor.getColumnIndex(TweetStore.Statuses.USER_PROFILE_IMAGE_URL);
            is_favorite = cursor.getColumnIndex(TweetStore.Statuses.IS_FAVORITE);
            is_retweet = cursor.getColumnIndex(TweetStore.Statuses.IS_RETWEET);
            is_gap = cursor.getColumnIndex(TweetStore.Statuses.IS_GAP);
            location = cursor.getColumnIndex(TweetStore.Statuses.LOCATION);
            is_protected = cursor.getColumnIndex(TweetStore.Statuses.IS_PROTECTED);
            is_verified = cursor.getColumnIndex(TweetStore.Statuses.IS_VERIFIED);
            in_reply_to_status_id = cursor.getColumnIndex(TweetStore.Statuses.IN_REPLY_TO_STATUS_ID);
            in_reply_to_user_id = cursor.getColumnIndex(TweetStore.Statuses.IN_REPLY_TO_USER_ID);
            in_reply_to_user_name = cursor.getColumnIndex(TweetStore.Statuses.IN_REPLY_TO_USER_NAME);
            in_reply_to_user_screen_name = cursor.getColumnIndex(TweetStore.Statuses.IN_REPLY_TO_USER_SCREEN_NAME);
            my_retweet_id = cursor.getColumnIndex(TweetStore.Statuses.MY_RETWEET_ID);
            retweet_id = cursor.getColumnIndex(TweetStore.Statuses.RETWEET_ID);
            retweet_timestamp = cursor.getColumnIndex(TweetStore.Statuses.RETWEET_TIMESTAMP);
            retweeted_by_user_id = cursor.getColumnIndex(TweetStore.Statuses.RETWEETED_BY_USER_ID);
            retweeted_by_user_name = cursor.getColumnIndex(TweetStore.Statuses.RETWEETED_BY_USER_NAME);
            retweeted_by_user_screen_name = cursor.getColumnIndex(TweetStore.Statuses.RETWEETED_BY_USER_SCREEN_NAME);
            user_id = cursor.getColumnIndex(TweetStore.Statuses.USER_ID);
            source = cursor.getColumnIndex(TweetStore.Statuses.SOURCE);
            retweet_count = cursor.getColumnIndex(TweetStore.Statuses.RETWEET_COUNT);
            favorite_count = cursor.getColumnIndex(TweetStore.Statuses.FAVORITE_COUNT);
            is_possibly_sensitive = cursor.getColumnIndex(TweetStore.Statuses.IS_POSSIBLY_SENSITIVE);
            is_following = cursor.getColumnIndex(TweetStore.Statuses.IS_FOLLOWING);
            medias = cursor.getColumnIndex(TweetStore.Statuses.MEDIAS);
            first_media = cursor.getColumnIndex(TweetStore.Statuses.FIRST_MEDIA);
            mentions = cursor.getColumnIndex(TweetStore.Statuses.MENTIONS);
        }
