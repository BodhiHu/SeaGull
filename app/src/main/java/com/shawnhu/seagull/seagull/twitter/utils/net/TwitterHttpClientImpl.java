...
import com.shawnhu.seagull.seagull.twitter.utils.net.ssl.HostResolvedSSLConnectionSocketFactory;
import com.shawnhu.seagull.seagull.twitter.utils.net.ssl.TwitterSSLSocketFactory;
 ...
 */
public class TwitterHttpClientImpl implements twitter4j.http.HttpClient, HttpResponseCode {
    private static final Logger logger = Logger.getLogger(TwitterHttpClientImpl.class);
    private final HttpClientConfiguration conf;
    private final CloseableHttpClient client;

    public TwitterHttpClientImpl(final Context context, final HttpClientConfiguration conf) {
        this.conf = conf;
        final HttpClientBuilder clientBuilder = HttpClients.custom();
        final LayeredConnectionSocketFactory factory = TwitterSSLSocketFactory.getSocketFactory(context,
    ...
    public twitter4j.http.HttpResponse request(final twitter4j.http.HttpRequest req) throws TwitterException {
        ...
        try {
            ...
            // TODO
            if (resolver instanceof TwitterHostAddressResolver) {
                final TwitterHostAddressResolver twidereResolver = (TwitterHostAddressResolver) resolver;
