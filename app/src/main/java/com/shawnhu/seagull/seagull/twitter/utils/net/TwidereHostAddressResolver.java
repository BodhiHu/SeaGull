...
public class TwidereHostAddressResolver implements HostAddressResolver {
	...
	public TwidereHostAddressResolver(final Context context, final boolean local_only) {
		mHostMapping = context.getSharedPreferences(SeagullTwitterConstants.HOST_MAPPING_PREFERENCES_NAME, Context.MODE_PRIVATE);
		mPreferences = context.getSharedPreferences(SeagullTwitterConstants.SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE);
		final String address = mPreferences.getString(SeagullTwitterConstants.KEY_DNS_SERVER, DEFAULT_DNS_SERVER_ADDRESS);
	...
	public String resolve(final String host) throws IOException {
		...
		if (mDns != null && mPreferences.getBoolean(SeagullTwitterConstants.KEY_TCP_DNS_QUERY, false)) {
